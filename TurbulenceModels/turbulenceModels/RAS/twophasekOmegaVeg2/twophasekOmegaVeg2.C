...
void twophasekOmegaVeg2<BasicTurbulenceModel>::correctNut()
{
    ...
    */
    this->nut_ = ks_/
    (
        max(omegas_, Clim_*sqrt((2.0*(magSqr(symm(fvc::grad(this->U_)))))/Cmu_))
    ) + 
    kw_/
    (
        max(omegaw_, Clim_*sqrt((2.0*(magSqr(symm(fvc::grad(this->U_)))))/Clambda_))
    )
    ;
...
template<class BasicTurbulenceModel>
twophasekOmegaVeg2<BasicTurbulenceModel>::twophasekOmegaVeg
(
...
void twophasekOmegaVeg2<BasicTurbulenceModel>::correct()
{
	...
	fvm::ddt(kw_)
      ...
      ==
        fvm::Sp(KE6_*ESD6_/kw_, kw_) //turbulent production by vegetation drag force
      + fvm::Sp(-Clambda_*omegaw_, kw_) //turbulent disspation by vegetation drag force
      //Spectral shortcut
    ...
    if (writeTke_)
    {
        #include "writeTKEBudget_kOmegaVeg2.H"
    }
    omegasEqn.ref().relax();
